...
func TestGetBranchInfo(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			info, err := git.GetBranchInfo(tc.repoDir, testFS)
			...
			if err != nil {
				t.Fatalf("GetBranchInfo(%q) failed: %v", tc.repoDir, err)
			}
...
func TestBranchName(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			testFS := testFileReader{files: tc.files}
			branch, err := git.BranchName(tc.repoDir, testFS)

			if tc.expectError {
				if err == nil {
					t.Fatalf("BranchName(%q, testFS) expected error but got none", tc.repoDir)
				}
				if tc.expectDetached && !errors.Is(err, git.ErrDetachedHead) {
					t.Errorf("BranchName(%q, testFS) expected ErrDetachedHead but got %v", tc.repoDir, err)
				}
				return
			}
...
func TestHeadDigest(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			testFS := testFileReader{files: tc.files}
			digest, err := git.HeadDigest(tc.repoDir, testFS)
			...
			if digest.String() != tc.expectedHash {
				t.Errorf("HeadDigest(%q) = %q; want %q", tc.repoDir, digest.String(), tc.expectedHash)
			}
		})
	}
}
