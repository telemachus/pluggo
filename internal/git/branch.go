...
// GetBranchInfo returns an error if the branch name or SHA digest cannot be
// determined.
func GetBranchInfo(repoDir string, fr FileReader) (BranchInfo, error) {
	var info BranchInfo
...
// BranchName returns the branch name for a git repository. BranchName returns
// an error if the branch name cannot be determined.
func BranchName(repoDir string, fr FileReader) (string, error) {
	br, err := getBranch(repoDir, fr)
	if err != nil {
		return "", err
	}
